...
class TestResultFailureComponentScreen extends StatefulWidget {
  final FailureComponentDataDTO? failureComponentDTO;
  final String? startTime;
  final String? endTime;
  final String? serialNumber;
  TestResultFailureComponentScreen(
      {Key? key,
  ...
  State<StatefulWidget> createState() {
    return _TestResultFailureComponentScreen();
  }
}

class _TestResultFailureComponentScreen
    extends State<TestResultFailureComponentScreen> {
  TestResultTestNameDTO? testResultDTO;
  bool isLoading = true;
  ...
  Future<TestResultTestNameDTO> getFailureComponents(BuildContext context) {
    String companyId = widget.failureComponentDTO!.companyId!;
    String siteId = widget.failureComponentDTO!.siteId!;
    String equipmentId = widget.failureComponentDTO!.equipmentId!;
    String projectId = widget.failureComponentDTO!.projectId!;
    String timestamp = widget.failureComponentDTO!.timestamp!;
    String testname = widget.failureComponentDTO!.testName!;
    String startTime = widget.startTime!;
    String endTime = widget.endTime!;
    String serialNumber = widget.serialNumber!;
    DqmApi dqmApi = DqmApi(context);
    return dqmApi.getFailureComponents(companyId, siteId, equipmentId,
        projectId, timestamp, testname, startTime, endTime, serialNumber);
  }
  ...
  void initState() {
    ...
    Utils.setFirebaseAnalyticsCurrentScreen(
        Constants.ANALYTICS_DQM_RMA_TESTRESULT_FAILURE_COMP_SCREEN);
    ...
    callGetFailureComponents(context);
  }

  callGetFailureComponents(BuildContext context) async {
    await getFailureComponents(context).then((value) {
      if (value.status!.statusCode == 200) {
        this.testResultDTO = value;
      } else {
  ...
  Widget chart(BuildContext ctx) {
    return Container(
      ...
      child: (this.testResultDTO != null &&
              this.testResultDTO!.data != null &&
              testResultDTO!.data!.length > 0)
          ? WebViewPlus(
                ...
                JavascriptChannel(
                    ...
                    onMessageReceived: (message) {
                      this.dqmWebViewController.webViewController.runJavascript(
                          'fetchRmaFailureComponentData(${jsonEncode(this.testResultDTO)}, "${widget.serialNumber}", "${Utils.getTranslated(ctx, 'chart_footer_timestamp_measured')}", "${Utils.getTranslated(ctx, 'chart_legend_pass')}", "${Utils.getTranslated(ctx, 'chart_legend_fail')}", "${Utils.getTranslated(ctx, 'chart_legend_anomaly')}", "${Utils.getTranslated(ctx, 'chart_legend_false_failure')}", "${Utils.getTranslated(ctx, 'chart_legend_lower_limit')}", "${Utils.getTranslated(ctx, 'chart_legend_upper_limit')}")');
                    }),
                JavascriptChannel(
            ...
            child: CupertinoActionSheetAction(
              onPressed: () async {
                ...
                bool result = await CSVApi.generateCSV(
                    this.testResultDTO!.data!, filename);
                Navigator.pop(popContext);
                if (result == true) {
                  setState(() {
                    isLoading = false;
